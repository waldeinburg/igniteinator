...
function convert_img() {
  ...
  in_name=$2
  out_name=$3
  f="$IMG_INPUT_DIR/$lang/$in_name.jpg"
  out="$IMG_OUTPUT_DIR/$lang/$out_name.png"
  echo "$f -> $out"
...
jq -r '
  .cards[] | "\(.id)@\(.name)"' "$DATA_FILE" | while read -r data; do
  id=$(cut -d'@' -f1 <<<"$data")
  name=$(cut -d'@' -f2 <<<"$data")
  for l in en de es fr; do
    convert_img "$l" "$name" "$id"
  done
done

for l in en de es fr; do
  convert_img "$l" "Hex" "hex"
done

# Trash only exists for German
convert_img "de" "Trash" "trash"
